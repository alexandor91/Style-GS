...
from utils.extractor_utils import VGG19Extractor
...
class StyleTransformer:
    ...
    def __init__(
        ...
        self.optimizer = None
        self.schedulers = {}
        self.vgg_extractor = VGG19Extractor(out_indices=[0, 5, 10, 19, 28])
    ...
    # def calc_adain_style_loss(self, style_img, rendered_img):
    #     pass
...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        ### reconstruction loss ###
        content_loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss

        ### style loss ###
        style_loss = 0
        source_features = extractor(image.unsqueeze(0), target_size)
        source_grams = []  # C*C
        for source_feature in source_features:
            source_grams.append(gram_matrix(source_feature))
        for source_feature, source_gram, target_feature, target_gram in zip(
            source_features, source_grams, target_features, target_grams
        ):
            style_loss += style_loss_adain(
                source_feature, source_gram, target_feature, target_gram
            )
