...
class GaussianSHModel:
    ...
    def __init__(
        ...
        self.feat_dim = feat_dim  # maybe higher feat_dim
        ...
        self.mlp_sh = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(feat_dim, 3 * (self.max_sh_degree + 1) ** 2 * self.n_offsets),
        ).cuda()
    ...
    def training_setup(self, training_args):
        ...
        if self.use_feat_bank:
            l = [
                ...
                {
                    "params": self.mlp_sh.parameters(),
                    "lr": training_args.mlp_sh_lr_init,
                    "name": "mlp_sh",
                },
                {
        ...
        self.mlp_sh_scheduler_args = get_expon_lr_func(
            lr_init=training_args.mlp_sh_lr_init,
            lr_final=training_args.mlp_sh_lr_final,
            lr_delay_mult=training_args.mlp_sh_lr_delay_mult,
            max_steps=training_args.mlp_sh_lr_max_steps,
        )
        if self.use_feat_bank:
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group["lr"] = lr
            if param_group["name"] == "mlp_sh":
                lr = self.mlp_sh_scheduler_args(iteration)
                param_group["lr"] = lr
    ...
    def save_mlp_checkpoints(self, path, mode="split"):  # split or unite
        ...
        if mode == "split":
            ...
            self.mlp_sh.eval()
            sh_mlp = torch.jit.trace(
                self.mlp_sh,
                (torch.rand(1, self.feat_dim + self.appearance_dim).cuda()),
            )
            sh_mlp.save(os.path.join(path, "sh_mlp.pt"))
            self.mlp_sh.train()
    ...
    def load_mlp_checkpoints(self, path, mode="split"):  # split or unite
        if mode == "split":
            ...
            self.mlp_sh = torch.jit.load(os.path.join(path, "sh_mlp.pt")).cuda()
            if self.use_feat_bank:
            ...
            self.mlp_sh.load_state_dict(checkpoint["sh_mlp"])
            if self.use_feat_bank:
